...
async def register_user(registration_data: RegistrationRequest, session: Session = Depends(get_session)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid birthday format. Use YYYY-MM-DD")
    ...
    if registration_data.startTime:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid start time format. Use HH:MM")

    if registration_data.endTime:
        try:
            end_time = time.fromisoformat(registration_data.endTime)
        except ValueError:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid end time format. Use HH:MM")
